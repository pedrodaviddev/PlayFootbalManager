...
public class PlayersOnMatchAdapter extends RecyclerView.Adapter<PlayersOnMatchAdapter.PlayersOnMatchHolder> {
    ...
    private void buildNotification(List<Player> list, List<Player> players) {
        ...
        if(players.size()>0 && !players.get(0).getNickname().equals(UserRepository.currentNickname)){
            ...
            list.removeAll(aux);
            if (list.size() > 0 && !list.get(0).getNickname().equals(UserRepository.currentNickname)) {
